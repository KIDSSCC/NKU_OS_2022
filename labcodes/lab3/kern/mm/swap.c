...
swap_out(struct mm_struct *mm, int n, int in_tick)
{
     ...
     for (i = 0; i != n; ++ i)
     {
          ...
          // 由swap置换管理器，挑选出需要被牺牲的(被置换到swap磁盘扇区)的page，令page指针变量指向其指针，这个page是当前在内存里的，住呢比被换出去
          ...
          assert((*ptep & PTE_P) != 0);
          // page->pra_vaddr/PGSIZE = 虚拟地址对应的二级页表项索引(前20位)；在第几个页，其实在后12位补零就是页的基址了，这里感觉和右移12位是一个性质。
...
check_swap(void)
{
    //backup mem env
     ...
     cprintf("在这里，%#x\n",mm->pgdir[PDX(BEING_CHECK_VALID_VADDR)]);
     ...
     cprintf("count is %d, total is %d,pgfault_num is %d\n",count,total,pgfault_num);
     //assert(count == 0);
